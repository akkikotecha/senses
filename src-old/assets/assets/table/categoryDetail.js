    ...
    success: function (result) {
      ...
      $.each(result, (i, val) => {
        ...
        items.push({
          ...
          related_product_id: JSON.stringify(val.related_product_id),
...
$("#grid").on("click", "button.edit_data", function () {
  ...
  var overview = $(this).attr("data-overview");
  // $(".ck-editor__editable_inline").text(overview);
  var testing = "<p><strong>Tilte</strong></p><p><i>123</i></p>";
  CKEDITOR.instances[editEditor].setData(testing);
  // theEditor.data.set(overview);
  ...
  $("#updatededit_featuredOneTitle").val(featureOneTitle);
  $("#updatededit_featuredTwoTitle").val(featureTwoTitle);
  $("#updatededit_featuredThreeTitle").val(featureThreeTitle);

  $("#updateeditproductMetaTitle").val($(this).attr("data-metaTitle"));
  $("#UpdateeditproductMetaDescription").val(
    $(this).attr("data-metaDescription")
  );

  // $("#select_multiple_dataData").val();
  // $("#mySelect2").trigger("change");
  var data = $(this).attr("data-related_product_id");
  console.log("TESING DATA ", data);
  // $("#select_multiple_dataData").val(data).trigger("change");
  var valueToSelect = data;

  // Get the Select2 element
  var $select2 = $("#select_multiple_dataData");

  // Unselect all current options (clear selections)
  $select2.val(null).trigger("change");

  // Loop through the options and select the ones with the desired values
  $select2.find("option").each(function () {
    if (valueToSelect.includes($(this).val())) {
