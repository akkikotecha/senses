...
ngAfterViewInit() {
  ...
  //   });
    ...
    .then((editor: { model: { document: { on: (arg0: string, arg1: (_evt: any, _data: any) => void) => void; }; }; getData: () => any; }) => {
        console.log('Editor initialized.');
...
submit() {
  ...
  if ($('#add-form').parsley().validate()) {
    ...
    if(this.description_value_1 ==="null" || this.description_value_1=== undefined || this.description_value_1 ===""){
      console.log(this.description_value_1);
      this.featuredOneDescValidation=true 
    }
    else{
      if($('#banner_index').val() == "" ||$('#banner_index').val() == "1" || $('#banner_index').val() == "2" || $('#banner_index').val() == "3")
      {
